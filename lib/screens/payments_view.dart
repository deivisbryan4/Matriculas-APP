...
class PaymentsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user.role == UserRole.student) {
      return _buildStudentPayments(context, system, isMobile);
    }
    return _buildAdminPayments(system, isMobile);
  }

  Widget _buildStudentPayments(BuildContext context, SystemProvider system, bool isMobile) {
    return SingleChildScrollView(
      padding: EdgeInsets.all(isMobile ? 20 : 32),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildPaymentForm(context, system, isMobile),
  ...
  Widget _buildPaymentForm(BuildContext context, SystemProvider system, bool isMobile) {
    return Card(
      child: Padding(
        padding: EdgeInsets.all(isMobile ? 20 : 32),
        child: Column(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      system.uploadedVoucher ?? 'Adjuntar voucher de pago',
                      ...
                      children: [
                        ...
                        if (v.statusEnum == PaymentStatus.pendiente) ...[
                            ...
                            children: [
                              Expanded(
                                child: OutlinedButton(
                                  onPressed: () => system.validateVoucher(
                                    v.id,
                                    'VALIDADO',
                                    '',
                                  ),
                          ...
                          Padding(
                            ...
                            child: _buildStatusBadge(v.statusEnum),
                          ),
                      ],
                    ),
                  ),
  ...
  Widget _buildStatusBadge(PaymentStatus status) {
    Color color = status == PaymentStatus.validado
        ? AppTheme.emeraldGreen
        : (status == PaymentStatus.rechazado
              ? AppTheme.roseRed
              : AppTheme.amberOrange);
    String text = status == PaymentStatus.validado
        ? 'Validado'
        : (status == PaymentStatus.rechazado ? 'Rechazado' : 'Pendiente');
    return Container(
